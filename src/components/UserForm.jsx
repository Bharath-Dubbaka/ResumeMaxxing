   ...
   const handleChange = (field, value) => {
      if (field === "experience") {
         ...
         if (titleChanges.length > 0) {
            const updatedCustomSkills = userDetails.customSkills?.map(
               (skill) => ({
                  ...skill,
                  experienceMappings: skill.experienceMappings.map((title) => {
                     const change = titleChanges.find(
                        (c) => c.oldTitle === title
                     );
      ...
      setUserDetails((prev) => ({
         ...
         customSkills: [
            ...(prev.customSkills || []),
            {
               skill: "",
               experienceMappings:
                  prev.experience?.map((exp) => exp.title) || [],
            },
         ],
      }));
   };
            ...
            <div className="max-w-5xl mx-1 md:mx-auto px-1 lg:px-10">
               <form onSubmit={handleSave} className="space-y-6">
                                    ...
                                    <CardContent className="p-6">
                                       ...
                                       <div className="space-y-4">
                                          <div className="flex items-center gap-4">
                                             <Label className="text-sm font-medium text-gray-700">
                                                Generate responsibilities based
                                                on:
                                             </Label>
                                             <Select
                           ...
                           <div className="flex flex-wrap gap-5 justify-start">
                              {userDetails?.customSkills?.map(
                                 (skillItem, index) => (
                                    <div
                                       key={index}
                                       className="w-[90%] md:w-[23%] group relative border border-slate-200 py-2 px-3 bg-purple-50 rounded-xl"
                                    >
                                                         ...
                                                         return (
                                                            ...
                                                            >
                                                               <input
                                                                  type="checkbox"
                                                                  id={`mapping-${index}-${i}`}
                                                                  checked={skillItem.experienceMappings.includes(
                                                                     exp.title
                                                                  )}
                                                                  disabled={
                                                                     isTitleBased
                  ...
                  <div className="sticky bottom-0 py-4 bg-white/80 backdrop-blur-xl border-t shadow-lg">
                     <div className="max-w-4xl mx-auto px-4 flex flex-col-reverse md:flex md:flex-row justify-end gap-4">
                        <Button
                           type="button"
                           variant="destructive"
                           onClick={onCancel}
