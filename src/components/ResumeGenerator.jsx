...
const ResumeGenerator = () => {
   ...
   const getAllSkills = () => {
      const mappedSkills =
         combinedSkills?.map((mapping) => mapping.skill) || [];
      const customSkills =
   ...
   const generateResponsibilities = async (experience) => {
      ...
      const relevantSkills = [
         ...new Set(
            combinedSkills
               .filter((skillObj) =>
                  skillObj.experienceMappings?.includes(experience.title)
               )
   ...
   const downloadAsWord = async (template) => {
      ...
      try {
         ...
         switch (template) {
            case "Standard":
               const { Standardformat } = await import(
                  "./templates/Standardformat"
               );
               doc = Standardformat(resumeData);
               break;
            case "Hybrid":
               const { Hybridformat } = await import(
                  "./templates/Hybridformat"
               );
               doc = Hybridformat(resumeData);
               break;
            case "ModernClean":
               const { ModernCleanFormat } = await import(
                  "./templates/ModernCleanFormat"
               );
               doc = ModernCleanFormat(resumeData);
               break;
            default:
               const { Standardformat: defaultFormat } = await import(
                  "./templates/Standardformat"
               );
      ...
      <Card className="bg-white/60 shadow-lg border-0 backdrop-blur-2xl rounded-xl mt-6">
         ...
         <CardContent>
            <div className="text-xs text-gray-600 pb-1">
               <p>
                  <strong>Note:</strong> Click <em>"Generate Resume"</em> to
                  create a resume based on the analyzed job description and
                  extracted skills, or generate it using only your saved custom
                  data (no job description required).
               </p>
            </div>
