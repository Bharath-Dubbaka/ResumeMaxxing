...
import { store } from "../store/store"; // ✅ access to redux store
import { setUserQuota } from "../store/slices/firebaseSlice";

export class QuotaService {
   static async getUserQuota() {
      const quota = await apiClient("/api/quota", "GET");
      store.dispatch(setUserQuota(quota));
      return quota;
   }

   static async incrementUsage(type) {
      const updatedQuota = await apiClient("/api/quota/increment", "POST", {
         type,
      });
      store.dispatch(setUserQuota(updatedQuota));
      return updatedQuota;
   }

   static async upgradeToPremium() {
      const upgraded = await apiClient("/api/quota/upgrade", "POST");
      store.dispatch(setUserQuota(upgraded));
      return upgraded;
   }
}
